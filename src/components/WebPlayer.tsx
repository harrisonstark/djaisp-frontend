...
function WebPlayback(props) {
    ...
    function playRandomTracks(message = ""){
        ...
        axios.get(`https://w6vivobrm3.loclx.io/get_recommendation?user_id=${user_id}&email=${email}&${queryParams}`)
        .then((response) => {
    ...
    useEffect(() => {
        ...
        .catch(error => {
            if(error.message === "Request failed with status code 404"){
                ...
                axios.put(`https://w6vivobrm3.loclx.io/authorize?user_id=${Cookies.get('user_id')}&email=${Cookies.get('email')}`)
                .then(() => {
